...
while (i < stdage.length) {
  console.log(stdage[i]);
  i++;
}

const even = stdage.filter((Number) => {
  return Number % 2 === 0;
});
console.log(even);
...
// why is the varibale outside the function?
...
var array = [
  3, 6, 67, 6, 23, 11, 100, 8, 93, 0,11, 17, 24, 7, 1, 33, 45, 28, 33, 23, 12, 99,
  100,
];
let indexNum = 2;
...
// console.log("repeated numbers :  ", arr);
console.log("findDuplicates(array); ",findDuplicates(array));


// findDuplicates(array);
//SOlution 2
function doubbleNum(arr) {
  const result = [];
  for (let i = 0; i < arr.length; i++) {
    for (let index = i + 1; index < arr.length; index++) {
      if (!result.includes(arr[i]) && arr[i] === arr[index]) {
...
function duplicates(array) {
  console.log(
    "map indexof",
    array.map((item, i) => array.indexOf(item))
  );
  let newArray = array.filter((item, index) => array.indexOf(item) !== index);
  return newArray;
}

console.log(" duplicates ", duplicates([1, 2, 3, 1, 3, 4, 5, 10, 10]));
...
//wrong, had another understanding of the excercise
...
function capitalizeFirstLetters(phrase) {
  return phrase // why is the return in this plac?
    .toLowerCase()
    .split(" ")
...
function longestword(str) {
  ...
  for (let i = 0; i < arraySentence.length; i++) {
    if (arraySentence[i].length > Maxwordlength.length) {
...
let str = "Web Development Tutorial abc";
longestword(str);
